...
public interface UsrViewMapper {
    ...
    @Select("select usr_id as usrId, usr_nm as name, email as mail from usr where email = #{email};")
    ...
    @Select("select fl_id as img from files where prd_id = #{prdId};")
    List<String> peekImg(String prdId);
    ...
    @Select("select max(f.fl_id) as img, w.win_price as win_price," +
